...
@st.cache_data
def cargar_datos():
    ruta_dist = DATA_DIR / "distancias.json"
    ruta_coords = DATA_DIR / "coordenadas.json"

    if not ruta_dist.exists() or not ruta_coords.exists():
        st.error(
            "No se encontraron los archivos de datos `distancias.json` y/o "
            "`coordenadas.json` junto a `app.py`. Verifica que ambos archivos "
            "estén subidos en la raíz del repositorio de GitHub (al mismo nivel "
            "que app.py, no dentro de una subcarpeta)."
        )
        st.stop()

    with open(ruta_dist, encoding="utf-8") as f:
        dist = json.load(f)
    with open(ruta_coords, encoding="utf-8") as f:
